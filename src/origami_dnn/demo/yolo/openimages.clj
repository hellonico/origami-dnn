(ns origami-dnn.demo.yolo.openimages
  (:require
...
(defn run-yolo [& args]
  (let [input (or (first args) "resources/catwalk.jpg")
        [net opts labels] (origami-dnn/read-net-from-repo "network.darknet:yolo-openimages:1.0.0")
...
(defn -main [& args]
  (run-yolo
   ...
   (or (second args) "openimages_output.jpg")))
